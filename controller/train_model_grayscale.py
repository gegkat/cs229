...
def generator(samples, output_throttle=False, batch_size=32):
    ...
    while 1: # Loop forever so the generator never terminates
        ...
        for offset in range(0, num_samples, batch_size):
            ...
            X_train = np.array(images)#.reshape(b,m,n,1)#RESHAPE DONE HERE
            #print("xtrain shape:"+str(np.shape(X_train)))
            b,m,n=np.shape(X_train)
            X_train=np.reshape(X_train,(b,m,n,1))
            #print("xtrain shape:"+str(np.shape(X_train)))
            if output_throttle:
...
if __name__ == '__main__':
    ...
    if args.model == "NVIDIA":
        ...
        model=models.complex_CNN(model,n_outputs)
    else: 
